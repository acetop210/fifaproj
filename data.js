...
function dictMake(){
  ...
  for(var i=0; div[i]; i++){
    div_dictp[div[i].divisionId] = div[i].divisionName;
  }
  for(let i=0; sea[i]; i++){
    sea_dict[sea[i].seasonId] = [sea[i].className, sea[i].seasonImg];
  }

  Object.keys(div_dictp).sort().forEach(function (key){
    div_dict[key] = div_dictp[key];
  });
...
function getUserInfo(id){
  httpDo(
    ...
    function(res) {
      ...
      divtable = new divTable(parseInt(userdata[0].division));
    }
  );
}
